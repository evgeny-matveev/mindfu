...
module MeditationPlayer
  class RandomFileSelectorTest < Test
    ...
    def test_excludes_recently_played_files_from_last_sessions
      # Mock persistence data with recently played files (basenames like stored in JSON)
    ...
    def test_saves_played_file_to_recently_played
      ...
      @player.stub(:audio_files, mock_files) do
        # Test saving played file with full path
        @selector.record_played_file("/fake/path/file1.mp3")

        # Verify it was saved as basename by checking if it's excluded from selection
        ...
        refute_equal "/fake/path/file1.mp3", selected_file unless mock_files.length == 1
      end
    end

    def test_limits_recently_played_to_90_percent_of_files
      # Create 20 files with full paths
      mock_files = (1..20).map { |i| "/fake/path/file#{i}.mp3" }

      @player.stub(:audio_files, mock_files) do
        # Play 18 files (90% of 20)
        18.times do |i|
          @selector.record_played_file("/fake/path/file#{i + 1}.mp3")
        end

        # Play one more file
        @selector.record_played_file("/fake/path/file19.mp3")

        # Test that files work correctly
        10.times do
          selected_file = @selector.select_random_file
    ...
    def test_session_history_tracks_current_session
      ...
      @player.stub(:audio_files, mock_files) do
        ...
        @selector.add_to_session_history("/fake/path/file1.mp3")
        @selector.add_to_session_history("/fake/path/file2.mp3")
    ...
    def test_next_selects_random_file_and_adds_to_history
      ...
      @player.stub(:audio_files, mock_files) do
        ...
        @selector.stub(:select_random_file, "/fake/path/file3.mp3") do
          ...
          assert_equal "/fake/path/file3.mp3", next_file

          # Verify it was added to session history
          history = @selector.session_history
          assert_includes history, "/fake/path/file3.mp3"
    ...
    def test_previous_navigates_session_history
      ...
      @player.stub(:audio_files, mock_files) do
        ...
        assert_equal "/fake/path/file2.mp3", prev_file

        # Test previous again
        prev_file = @selector.previous_file
        assert_equal "/fake/path/file1.mp3", prev_file
      end
    end

    def test_previous_returns_nil_when_no_history
      mock_files = ["/fake/path/file1.mp3", "/fake/path/file2.mp3"]
    ...
    def test_initializes_with_random_file
      ...
      @player.stub(:audio_files, mock_files) do
        @selector.stub(:select_random_file, "/fake/path/file2.mp3") do
          # Clear recently played files for this test
          @selector.instance_variable_set(:@recently_played_files, [])

          initial_file = @selector.initialize_session

          assert_equal "/fake/path/file2.mp3", initial_file

          # Verify it was added to session history
          history = @selector.session_history
          assert_includes history, "/fake/path/file2.mp3"
